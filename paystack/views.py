...
def verify_payment(request, order):
    ...
    if response[0]:
        payment_verified.send(
            ...
            order=order)
        # return redirect('orders:paysuccess', args=[order])
        return redirect(
...
def update_payment(json_body):
    ...
    # metadata = ndata['customer']['metadata']
    # print(metadata)
    ...
    if event == 'charge.success':
        ...
        try:
            ...
            subject = f'Myshop - Invoice NO. {order.id}'
            message = 'Please find attached the invoice for your recent purchase'
            email = EmailMessage(subject, message, 'ishaqmuhammed191@gmail.com', [order.email])
            ...
            print('Not Sent, SMTPAuthenticationError')
